...
const SpecRow = ({ label, value, isLast, index }) => (
  <View style={[styles.specRow, index % 2 === 0 && styles.specRowAlt, isLast && { borderBottomWidth: 0 }]}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.topSection}>
          ...
          <View style={styles.topRatingRow}>
            ...
            <View style={[styles.saleStatusBadge, outOfStock && styles.saleStatusBadgeOut]}>
              <Text style={[styles.saleStatusText, outOfStock && styles.saleStatusTextOut]}>
                {stockLabel}
              </Text>
            </View>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.thumbnailScroll}>
            {THUMBNAIL_IMAGES.map((img, idx) => (
              <TouchableOpacity
                ...
                style={[styles.thumbnail, currentImageIndex === idx && styles.thumbnailActive]}
              >
          ...
          <View style={styles.flashBanner}>
            ...
            <View style={styles.flashTimeRow}>
              {FLASH_COUNTDOWN.map((item) => (
                <FlashTimeBox key={item.key} value={item.value} label={item.label} />
              ))}
            </View>
          </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoCardContent}>
            {UU_DAI.map((text, i) => <BenefitRow key={i} text={text} />)}
          </View>
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoCardDivider} />
          <View style={styles.infoCardContent}>
            {TRO_GIA.map((text, i) => <BenefitRow key={i} text={text} />)}
          </View>
        </View>
        ...
        <View style={styles.ctaWrap}>
          <TouchableOpacity
            ...
            disabled={outOfStock}
          >
            <Text style={styles.btnMainText}>{outOfStock ? 'HẾT HÀNG' : 'MUA NGAY'}</Text>
            <Text style={styles.btnMainSub}>( Giao tận nơi hoặc nhận tại cửa hàng )</Text>
          </TouchableOpacity>

          <View style={styles.btnRow}>
            <TouchableOpacity activeOpacity={0.85} style={styles.btnOutline}>
              <Text style={styles.btnOutlineText}>TRẢ GÓP</Text>
              <Text style={styles.btnOutlineSub}>( Thủ tục nhanh chóng nhận máy ngay )</Text>
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.85} style={styles.btnOutline}>
              <Text style={styles.btnOutlineText}>TRẢ GÓP QUA THẺ</Text>
              <Text style={styles.btnOutlineSub}>Visa, Mastercard, JCB, Amex</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {combos.map((item, i) => (
            <TouchableOpacity key={item.id} activeOpacity={0.8} onPress={() => toggleCombo(item.id)}>
          ...
          <View style={styles.comboTotalRow}>
            ...
            <Text style={styles.comboTotalValue}>{formatPrice(comboTotal)}</Text>
          </View>
        ...
        <View style={styles.section}>
          ...
          {specs.map((s, i) => (
            <SpecRow key={i} label={s.label} value={s.value} isLast={i === specs.length - 1} index={i} />
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.productImageBox}>
            <Image source={{ uri: product.image }} style={styles.highlightImage} resizeMode="contain" />
          </View>
          <Text style={styles.expertTitle}>{product.brand} - Hiệu năng mạnh mẽ cho học tập và gaming</Text>
          <Text style={styles.expertBody}>
            Sản phẩm được tối ưu hiệu năng để đáp ứng tốt nhu cầu học tập, làm việc lẫn giải trí.
            Tản nhiệt ổn định, thiết kế hiện đại và cấu hình cân bằng giúp máy hoạt động mượt mà trong thời gian dài.
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.ratingOverview}>
            <View style={styles.ratingLeft}>
              ...
              <Text style={[styles.stars, { fontSize: 18 }]}>{'★'.repeat(Math.round(product.rating || 0))}</Text>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', backgroundColor: '#fff', },
  headerBtn: { padding: 4 },
  headerCartBtn: { padding: 4, position: 'relative' },
  headerBtnIcon: { fontSize: 28, color: '#1a1a1a', lineHeight: 30 },
  headerTitle: { flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '700', color: '#1a1a1a', marginHorizontal: 10, },
  cartBadge: { position: 'absolute', top: -2, right: -4, backgroundColor: '#E53935', borderRadius: 10, minWidth: 16, height: 16, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 3, },
  cartBadgeText: { fontSize: 9, color: '#fff', fontWeight: '900' },
  topSection: { backgroundColor: '#FFFFFF', paddingHorizontal: 12, paddingVertical: 14, },
  topProductName: { fontSize: 13, color: TEXT_MAIN, lineHeight: 18, marginBottom: 10, fontWeight: '600', },
  topRatingRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 4, },
  topStars: { color: '#CCCCCC', fontSize: 14, letterSpacing: 2, },
  topReviewCount: { fontSize: 12, color: TEXT_MUTED, marginHorizontal: 4, },
  saleStatusBadge: { backgroundColor: ORANGE_LIGHT, borderRadius: 3, paddingHorizontal: 8, paddingVertical: 3, marginLeft: 4, },
  ...
  mainImageContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 10, },
  arrowBtn: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: '#F5F5F5', },
  arrowText: { fontSize: 20, color: TEXT_MAIN, fontWeight: '600', },
  mainImageWrap: { flex: 1, backgroundColor: '#F8F8F8', borderRadius: 8, height: 240, alignItems: 'center', justifyContent: 'center', },
  mainImage: { width: '90%', height: '90%', },
  thumbnailScroll: { marginBottom: 12, },
  thumbnail: { width: 60, height: 60, borderRadius: 6, backgroundColor: '#F5F5F5', alignItems: 'center', justifyContent: 'center', marginRight: 8, borderWidth: 1.5, borderColor: '#EBEBEB', },
  thumbnailActive: { borderColor: ORANGE, backgroundColor: '#FFF8F3', },
  thumbnailImage: { width: '85%', height: '85%', },
  actionButtonsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#F0F0F0', paddingTop: 10, },
  ...
  section: { backgroundColor: '#FFFFFF', paddingHorizontal: 14, paddingVertical: 14, },
  divider: { height: 8, backgroundColor: '#F2F2F2', },
  sectionTitle: { fontSize: 15, fontWeight: '600', color: TEXT_MAIN, marginBottom: 12, },
  stars: { color: '#F5A623', fontSize: 13, letterSpacing: 1, },
  flashBanner: { backgroundColor: '#F24A1E', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, },
  flashLeft: { flex: 1, },
  flashTitle: { color: '#FFFFFF', fontSize: 21, fontWeight: '800', lineHeight: 24, },
  flashSubTitle: { color: '#FFFFFF', fontSize: 10, fontWeight: '700', letterSpacing: 0.2, },
  flashTimeRow: { flexDirection: 'row', alignItems: 'center', gap: 4, },
  flashTimeBox: { width: 36, borderRadius: 4, backgroundColor: '#FFFFFF', alignItems: 'center', paddingVertical: 4, },
  flashTimeValue: { fontSize: 14, fontWeight: '700', color: '#F24A1E', lineHeight: 16, },
  flashTimeLabel: { fontSize: 8, fontWeight: '700', color: '#F24A1E', lineHeight: 10, },
  priceRow: { flexDirection: 'row', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 8, },
  price: { fontSize: 26, fontWeight: '700', color: ORANGE, },
  originalPrice: { fontSize: 13, color: TEXT_MUTED, textDecorationLine: 'line-through', },
  vatText: { fontSize: 13, color: ORANGE, fontWeight: '500', },
  infoCard: { backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F2C2AD', borderRadius: 10, marginHorizontal: 14, marginTop: 12, marginBottom: 8, overflow: 'hidden', },
  infoCardHeader: { backgroundColor: ORANGE_LIGHT, paddingHorizontal: 14, paddingVertical: 14, },
  infoCardDivider: { height: 1, backgroundColor: '#F2C2AD', },
  infoCardContent: { backgroundColor: '#FFFFFF', paddingHorizontal: 14, paddingVertical: 14, },
  blockTitle: { fontSize: 18, fontWeight: '900', color: ORANGE, letterSpacing: 0.4, },
  benefitRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, paddingVertical: 4, },
  checkCircle: { width: 18, height: 18, borderRadius: 9, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center', marginTop: 1, flexShrink: 0, },
  checkTick: { color: '#FFFFFF', fontSize: 10, fontWeight: '700', lineHeight: 13, },
  benefitText: { flex: 1, fontSize: 13, color: '#5D2E0A', lineHeight: 20, },
  serviceRow: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', backgroundColor: '#FFFFFF', paddingVertical: 10, paddingHorizontal: 14, },
  serviceItem: { flexDirection: 'row', alignItems: 'center', gap: 5, flex: 1, justifyContent: 'center', },
  serviceIcon: { fontSize: 14, },
  serviceText: { fontSize: 12, color: TEXT_MUTED, },
  serviceDivider: { width: 1, height: 18, backgroundColor: DIVIDER_COLOR, marginHorizontal: 8, },
  ctaWrap: { backgroundColor: '#FFFFFF', paddingHorizontal: 14, paddingBottom: 14, paddingTop: 4, },
  btnMain: { backgroundColor: ORANGE, borderRadius: 8, paddingVertical: 14, alignItems: 'center', marginBottom: 8, },
  btnMainText: { color: '#FFFFFF', fontSize: 15, fontWeight: '700', letterSpacing: 0.5, },
  btnMainSub: { color: 'rgba(255,255,255,0.85)', fontSize: 11, marginTop: 2, },
  btnRow: { flexDirection: 'row', gap: 8, },
  btnOutline: { flex: 1, borderWidth: 1.5, borderColor: ORANGE, borderRadius: 8, paddingVertical: 10, paddingHorizontal: 8, alignItems: 'center', },
  btnOutlineText: { color: ORANGE, fontSize: 12, fontWeight: '700', letterSpacing: 0.3, },
  btnOutlineSub: { color: TEXT_MUTED, fontSize: 10, marginTop: 2, textAlign: 'center', },
  comboItem: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: DIVIDER_COLOR, },
  checkbox: { width: 18, height: 18, borderRadius: 4, borderWidth: 1.5, borderColor: '#CCCCCC', alignItems: 'center', justifyContent: 'center', flexShrink: 0, },
  checkboxChecked: { backgroundColor: ORANGE, borderColor: ORANGE, },
  checkboxTick: { color: '#FFFFFF', fontSize: 11, fontWeight: '700', lineHeight: 14, },
  comboImageBox: { width: 52, height: 52, borderRadius: 6, backgroundColor: '#F5F5F5', alignItems: 'center', justifyContent: 'center', flexShrink: 0, borderWidth: 0.5, borderColor: DIVIDER_COLOR, },
  comboEmoji: { fontSize: 22, },
  comboInfo: { flex: 1, },
  comboName: { fontSize: 12, color: TEXT_MAIN, lineHeight: 17, marginBottom: 4, },
  comboPrice: { fontSize: 13, color: ORANGE, fontWeight: '600', },
  comboTotalRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTopWidth: 1, borderTopColor: DIVIDER_COLOR, marginTop: 2, },
  comboTotalLabel: { fontSize: 13, color: TEXT_MUTED, },
  comboTotalValue: { fontSize: 16, fontWeight: '700', color: ORANGE, },
  comboSavingText: { marginTop: 6, fontSize: 12, color: '#16A34A', fontWeight: '600', },
  specRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, paddingHorizontal: 8, borderBottomWidth: 0.5, borderBottomColor: DIVIDER_COLOR, },
  specRowAlt: { backgroundColor: '#F3F4F6', },
  specLabel: { width: '38%', fontSize: 13, color: '#4B5563', fontWeight: '700', lineHeight: 20, },
  specValue: { flex: 1, fontSize: 13, color: '#6B7280', lineHeight: 20, },
  productImageBox: { backgroundColor: '#F5F5F5', borderRadius: 10, height: 160, alignItems: 'center', justifyContent: 'center', marginBottom: 12, },
  highlightImage: { width: '90%', height: '90%', },
  expertTitle: { fontSize: 13, fontWeight: '600', color: TEXT_MAIN, marginBottom: 6, },
  expertBody: { fontSize: 13, color: TEXT_MUTED, lineHeight: 21, },
  ratingOverview: { flexDirection: 'row', alignItems: 'center', gap: 16, paddingVertical: 8, marginBottom: 12, },
  ratingLeft: { alignItems: 'center', gap: 4, },
  ratingBig: { fontSize: 40, fontWeight: '700', color: TEXT_MAIN, lineHeight: 46, },
  ...
  ratingBarTrack: { flex: 1, height: 4, backgroundColor: '#EBEBEB', borderRadius: 2, },
  ratingBarFill: { height: 4, width: 0, backgroundColor: '#F5A623', borderRadius: 2, },
