...
const ProductDetailScreen = ({ route, navigation }) => {
  ...
  const getComboSummary = () => {
    ...
    for (const item of combos) {
      if (item.checked) {
        total  += item.price;
        saving += Math.max(0, (item.listPrice || item.price) - item.price);
      }
    }
    return { total, saving };
  };

  const { total: comboTotal, saving: comboSaving } = getComboSummary();

  // ── addToCart: truyền object có đúng field `id` cho API ──────────────────
  const handleAddToCart = () => {
    addToCart(product);   // product.id đã được normalize đúng
  };
  ...
  const handlePrevImage = () => {
    setCurrentImageIndex(prev =>
      prev === 0 ? THUMBNAIL_IMAGES.length - 1 : prev - 1
    );
  };

  const handleNextImage = () => {
    setCurrentImageIndex(prev =>
      prev === THUMBNAIL_IMAGES.length - 1 ? 0 : prev + 1
    );
  };
...
const styles = StyleSheet.create({
  ...
  discountBadge: { backgroundColor: ORANGE, borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
  discountBadgeText: { color: "#fff", fontSize: 11, fontWeight: "700" },
  vatText:       { fontSize: 13, color: ORANGE, fontWeight: "500" },
  descriptionText: { fontSize: 13, color: TEXT_MUTED, lineHeight: 20, marginTop: 4 },
